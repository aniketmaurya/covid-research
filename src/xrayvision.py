...
class Merge_Dataset(Dataset):
    def __init__(self, datasets, seed=0, label_concat=False):
        ...
        for i, dataset in enumerate(datasets):
            self.which_dataset = np.concatenate(
                [self.which_dataset,
                 np.zeros(len(dataset)) + i])
            self.length += len(dataset)
            self.offset = np.concatenate(
                [self.offset,
                 np.zeros(len(dataset)) + currentoffset])
        ...
        if label_concat:
            new_labels = np.zeros([
                self.labels.shape[0], self.labels.shape[1] * len(datasets)
            ]) * np.nan
            for i, shift in enumerate(self.which_dataset):
                size = self.labels.shape[1]
                new_labels[i,
                           shift * size:shift * size + size] = self.labels[i]
    ...
    def __getitem__(self, idx):
        item = self.datasets[int(
            self.which_dataset[idx])][idx - int(self.offset[idx])]
    ...
    def __init__(self, dataset, labels=None):
        ...
        if labels:
            for label in labels:
                ...
                self.idxs += list(
                    np.where(dataset.labels[:,
                                            list(dataset.pathologies).
                                            index(label)] == 1)[0])
...
class NIH_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        self.pathology_maskscsv = pd.read_csv(
            ...
            names=[
                "Image Index", "Finding Label", "x", "y", "w", "h", "_1", "_2",
                "_3"
            ],
            skiprows=1)

        # change label name to match
        self.pathology_maskscsv["Finding Label"][
            self.pathology_maskscsv["Finding Label"] ==
...
class RSNA_Pneumonia_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        self.dicomcsv = pd.read_csv(self.dicomcsvpath,
                                    nrows=nrows,
                                    index_col="PatientID")
    ...
    def get_mask_dict(self, image_name, this_size):
        ...
        images_with_masks = self.raw_csv[self.raw_csv["patientId"] ==
                                         image_name]
        path_mask = {}
...
class NIH_Google_Dataset(Dataset):
    ...
    def __init__(self,
                 ...
                 csvpath=os.path.join(
                     thispath, "google2019_nih-chest-xray-labels.csv.gz"),
                 views=["PA"],
                 transform=None,
                 data_aug=None,
                 nrows=None,
        ...
        # rename pathologies
        self.pathologies = np.char.replace(self.pathologies,
...
class PC_Dataset(Dataset):
    ...
    def __init__(
        ...
        self.pathologies = [
            ...
            "Edema", "Emphysema", "Fibrosis", "Effusion", "Pneumonia",
            "Pleural_Thickening", "Cardiomegaly", "Nodule", "Mass", "Hernia",
            "Fracture", "Granuloma", "Flattened Diaphragm", "Bronchiectasis"
        ]
        ...
        mapping["Infiltration"] = [
            "infiltrates", "interstitial pattern", "ground glass pattern",
            "reticular interstitial pattern",
            "reticulonodular interstitial pattern", "alveolar pattern",
        ...
        self.csv.loc[self.csv["Projection"].isin(["AP_horizontal"]),
...
class MIMIC_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path = os.path.join(self.imgpath, "p" + subjectid[:2],
                                "p" + subjectid, "s" + studyid,
                                dicom_id + ".jpg")
        img = imread(img_path)
        img = normalize(img, self.MAXVAL)
...
class Openi_Dataset(Dataset):
    ...
    def __init__(self,
                 ...
                 tsnepacsv_path=os.path.join(thispath,
        ...
        for filename in tarf.getnames():
            if (filename.endswith(".xml")):
                ...
                labels_m = [
                    node.text.lower() for node in root.findall(".//MeSH/major")
                ]
                labels_m = "|".join(np.unique(labels_m))
                labels_a = [
                    node.text.lower()
                    for node in root.findall(".//MeSH/automatic")
                ]
        ...
        self.dicom_metadata = pd.read_csv(dicomcsv_path,
                                          index_col="imageid",
                                          low_memory=False)
...
class COVID19_Dataset(Dataset):
    ...
    def __init__(self,
                 ...
                 csvpath=os.path.join(thispath, "covid-chestxray-dataset",
                                      "metadata.csv"),
                 semantic_masks_v7labs_lungs_path=os.path.join(
                     thispath, "data", "semantic_masks_v7labs_lungs.zip"),
                 views=["PA", "AP"],
                 transform=None,
                 data_aug=None,
                 nrows=None,
...
class XRayResizer(object):
    ...
    def __call__(self, img):
        if self.engine == "skimage":
            ...
            return cv2.resize(img[0, :, :], (self.size, self.size),
                              interpolation=cv2.INTER_AREA).reshape(
                                  1, self.size, self.size).astype(np.float32)
        else:
            raise Exception(
...
class CovariateDataset(Dataset):
    ...
    def __init__(self,
        ...
        # TRAIN
        train_0_neg = np.random.choice(all_0_neg,
                                       int(n_per_category * ratio * splits[0] *
                                           2),
                                       replace=False)
        train_0_pos = np.random.choice(all_0_pos,
                                       int(n_per_category * (1 - ratio) *
                                           splits[0] * 2),
                                       replace=False)
        train_1_neg = np.random.choice(all_1_neg,
                                       int(n_per_category * (1 - ratio) *
                                           splits[0] * 2),
                                       replace=False)
        train_1_pos = np.random.choice(all_1_pos,
                                       int(n_per_category * ratio * splits[0] *
                                           2),
                                       replace=False)
        ...
        # VALID
        valid_0_neg = np.random.choice(all_0_neg,
                                       int(n_per_category * (1 - ratio) *
                                           splits[1] * 2),
                                       replace=False)
        valid_0_pos = np.random.choice(all_0_pos,
                                       int(n_per_category * ratio * splits[1] *
                                           2),
                                       replace=False)
        valid_1_neg = np.random.choice(all_1_neg,
                                       int(n_per_category * ratio * splits[1] *
                                           2),
                                       replace=False)
        valid_1_pos = np.random.choice(all_1_pos,
                                       int(n_per_category * (1 - ratio) *
                                           splits[1] * 2),
                                       replace=False)
        ...
        if mode == "train":
            (a, b, c, d) = _reduce_nsamples(nsamples, train_0_neg, train_0_pos,
                                            train_1_neg, train_1_pos)
        elif mode == "valid":
            (a, b, c, d) = _reduce_nsamples(nsamples, valid_0_neg, valid_0_pos,
                                            valid_1_neg, valid_1_pos)
        elif mode == "test":
            (a, b, c, d) = _reduce_nsamples(nsamples, test_0_neg, test_0_pos,
